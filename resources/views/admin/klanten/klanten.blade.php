<x-layout>
    <x-slot name="header">
        <h1 class="display-4 fw-bold">Klanten</h1>
    </x-slot>
    ...
    <x-slot>
        <div class="medewerkers-table-wrapper">
            <div class="before-medewerkers-table">
                ...
                <a href="{{ route('admin.klanten.create') }}" class="create-employee-button">+</a>
            </div>
        ...
        <thead>
            <tr>
                <th class="px-4 py-2 text-left">Naam klant</th>
        ...
        <tbody>
         ...
         @forelse($klanten as $klant)
                <tr>
                    <td class="px-4 py-2">{{ $klant->name }}</td>
                    <td class="px-4 py-2">{{ $klant->phone }}</td>
                    <td class="px-4 py-2">{{ $klant->email }}</td>
                    <td>
                        <div class="medewerkers-table-buttons">
                        <!-- Edit knop -->
                        <a href="{{ route('admin.klanten.edit', $klant->id) }}" class="medewerkers-table-button">
                        ...
                        <!-- Delete formulier -->
                        <form method="POST"action="{{ route('admin.klanten.destroy', $klant->id) }}">
                            @csrf
                            @method('DELETE')
                 ...
                 @empty
            <div>
             <p class="px-4">Druk op het plusje om een klant aan te maken.</p>
            </div>
            @endforelse
        </tbody>
    </table>
