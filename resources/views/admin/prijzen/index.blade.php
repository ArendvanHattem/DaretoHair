<x-layout>
    <x-slot name="header">
        <h1 class="display-4 fw-bold">Prijzen</h1>
    </x-slot>
    ...
    <x-slot>
        <div class="medewerkers-table-wrapper">
            <div class="before-medewerkers-table">
                ...
                <a href="{{ route('admin.prijzen.create') }}" class="create-employee-button">+</a>
            </div>
        ...
        <thead>
            <tr>
                <th class="px-4 py-2 text-left">Naam service</th>
        ...
        <tbody>
         ...
         @forelse($prices as $price)
                <tr>
                    <td class="px-4 py-2">{{ $price->service }}</td>
                    <td class="px-4 py-2">{{ $price->description }}</td>
                    <td class="px-4 py-2">{{ $price->amount }}</td>
                    <td class="px-4 py-2">{{ $price->category }}</td>
                    <td>
                        <div class="medewerkers-table-buttons">
                        <!-- Edit knop -->
                        <a href="{{ route('admin.prijzen.edit', $price->id) }}" class="medewerkers-table-button">
                        ...
                        <!-- Delete formulier -->
                        <form method="POST"action="{{ route('admin.prijzen.destroy', $price->id) }}">
                            @csrf
                            @method('DELETE')
